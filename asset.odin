...
load_gltf_float3_to_half4 :: proc(attrib: ^cgltf.attribute) -> [dynamic]half4 {
    ...
    // Build up buffer, appending a 1.0 to turn each float3 into a float4
    // while also converting to f16
    for i in 0..<attrib.data.count {
        pos := ptr[i]
        data[i] = {f16(pos.x), f16(pos.y), f16(pos.z), 1.0}
    }
...
DDSHeader_DXT10 :: struct {
    ...
    misc_flag: D3D11_RESOURCE_MISC_FLAGS,
    array_size: u32,
    misc_flags2: u32
}
...
D3D11_RESOURCE_MISC_FLAG :: enum u32 {
...
D3D11_RESOURCE_MISC_FLAGS :: bit_set[D3D11_RESOURCE_MISC_FLAG]
...
dxgi_to_vulkan_format :: proc(format: DXGI_FORMAT) -> vk.Format {
